...
public static class Api
{
    extension(IEndpointRouteBuilder routes)
    {
        ...
        {
            routes.MapGet("/shows", async (ShowsData service, CancellationToken token) =>
                await service.GetAllShowsAsync(token));

            // GET /shows/83c2019d-7e33-4abe-9751-7703a73a03f5
            routes.MapGet("/shows/{id:guid}", async (Guid id, ShowsData service) =>
            {
            ...
            routes.MapPost("/shows", async (ShowCreateRequest request, ShowsData service) =>
            {
